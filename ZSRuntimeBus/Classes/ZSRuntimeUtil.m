...
bool zs_replaceMethodWithMethod(Class originalClass, SEL originalSelector, Class swizzledClass, SEL swizzledSelector) {
...
bool zs_replaceMethodWithMethodType(Class originalClass, SEL originalSelector, bool originIsClassMethod, Class swizzledClass, SEL swizzledSelector, bool swizzledIsClassMethod) {
    NSCParameterAssert(originalClass);
    NSCParameterAssert(originalSelector);
    NSCParameterAssert(swizzledClass);
    NSCParameterAssert(swizzledSelector);
...
void zs_enumerateClassList(void(^handler)(Class aClass)) {
...
void zs_enumerateProtocolList(void(^handler)(Protocol *protocol)) {
    NSCParameterAssert(handler);
...
bool zs_classIsSubclassOfClass(Class aClass, Class parentClass) {
...
bool zs_classIsCustomClass(Class aClass) {
    NSCParameterAssert(aClass);
    if (!aClass) {
        return false;
    }
...
bool zs_classSelfImplementingMethod(Class aClass, SEL method, bool isClassMethod) {
    NSCParameterAssert(aClass);
...
bool zs_isObjcProtocol(id protocol) {
...
bool zs_protocolConformsToProtocol(Protocol *protocol, Protocol *parentProtocol) {
    unsigned int count;
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (parent == parentProtocol) {
            result = YES;
            break;
        } else if (zs_protocolConformsToProtocol(parent, parentProtocol)) {
            result = YES;
            break;
        }
    }
...
Protocol *_Nullable zs_objcProtocol(id protocol) {
    if (zs_isObjcProtocol(protocol)) {
...
BOOL zs_canEnumerateClassesInImage() {
...
void zs_enumerateClassesInMainBundleForParentClass(Class parentClass, void(^handler)(__unsafe_unretained Class aClass)) {
    if (handler == nil) {
        return;
    }
...
void zs_enumerateClassMethodsForClass(Class aClass, void(^handler)(Method classMethod))
{
    Class metaClass = object_getClass([aClass class]);
    unsigned int count;
